...
{
    queue<piii>Q;
    Q.push(piii(0, pii(sourceX, sourceY)));
    ...
    while(!Q.empty())
    {
        ...
        for(int i = 0; i < 8; i++)
        {
            ...
            if(Check(u, v))
            {
                ...
                {
                    if( dist[u][v] > dist[x][y] && !visited[u][v])
                    {
                        dist[u][v] = dist[x][y];
                        visited[u][v] = 1;
                        Q.push(piii(-dist[u][v], pii(u, v)));
                    }
                }
            }
        }
        for(auto it: P[x][y])
        {
            if( dist[it.first][it.second] > dist[x][y] + 1  )
            {
                ...
                Q.push(piii(-dist[it.first][it.second], pii(it.first, it.second)));
            }
        }
    }
    return -1;
